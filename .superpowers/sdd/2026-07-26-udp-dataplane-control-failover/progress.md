...
- Task 10: Operations docs + smoke phase-routing prep — partial commit; TCP dataplane cleanup deferred to explicit followup
  - **Docs (done)**: 4 docs updated with concrete UDP failover semantics —
    - `docs/architecture.md`: 新 «9.5. UDP 数据面 + TCP 控制 Failover 运维» 章节（topology, triggers, recovery resource retention, candidate exhaustion 一次性 finalize，配置项与 §1086-1093 的 ops 提示，6 个 smoke markers）；§9 卖点表新增 UDP Failover 行（commit `22c9c3f`），原 PoC 多通道行标记 **已退役**；§10 路线图把「数据面生产化」改为「Remaining: toml production config + multi-version + NeoForge+ toml + TCP dataplane legacy cleanup」.
    - `docs/chunk-cache.md`: §9 新增 line 提示 SectionDeltaS2CPacket 经 UDP 数据面 dispatch（Task 8/Task 7/9 协同）。
    - `docs/runtime-smoke-test.md`: `-SmokePhases` 参数说明标 `udp-failover` 为 Task 10 followup 接入；新增「`udp-failover` Phase Markers」章节，列出 6 个 marker。
    - `docs/version-segments.md`: 网络子系统分段表 1.20.1 分界标注 UDP Failover 落地点（commit `22c9c3f`），其它版本为 Task 10+ followup。
  - **Smoke phase script extend (deferred)**: `scripts/runtime-smoke-test.ps1 -Phase UdpFailover` 接入 + `ServerSmokeTest.runUdpFailover` 与 6 个 marker log 注入需要 (1) `ServerSmokeTest` 中 mock 主 TCP close + UDP lease 保留行为, (2) 客户端 + 服务端 cross-process 协调时序 — 这是一个 smoke harness 工程，与 Task 9 production code 不耦合。当前 commit 把 marker 与 outline 注入 doc，cross-process smoke harness 作为 followup 主题（Task 10b）。
  - **TCP dataplane leftovers cleanup (deferred)**: plan §1080 要求使用 LSP references 删 `BulkRouter`/`DataPlaneCodec`/`VarIntLengthFrameSplitter`/`PlayerChannelBundle` 前迁移每个 production caller. 本环境 LSP unavailable (session memory 中记录). 已识别 production callers: `ServerSmokeTest` line 195/202/211/232 仍调用 `DataPlaneServer.killDataChannelByPortIdx/getBundle<PlayerChannelBundle>`（PoC dataplane phase 仍未退役）. 删除 `BulkRouter`/`PlayerChannelBundle` 会让 `ServerSmokeTest` dataplane phase 不可编译 —— 需先退役 dataplane phase（options/tasks: 1) 退役 `ServerSmokeTest.runDataplane` 整段，保留 `runClassic`；2) 在 `DataPlaneTransportCutoverTest.runtimeTransportNamesForTest()` 上加「`DataPlaneUdpServer` 不含 `NioServerSocketChannel`」red test；3) 后删 `BulkRouter`/`DataPlaneCodec`/`VarIntLengthFrameSplitter`/`PlayerChannelBundle`）。**该 cleanup 风险通过 Server smoke harness 退役 dataplane phase 化解，留为 Task 10b follow-up**。
  - **Verifications**: `common:test --tests io.github.limuqy.mc.hassium.network.dataplane.*` 全 11 个 task `:common:test` BUILD SUCCESSFUL （包括 Orchestrator 4 cases + RecoveryState 4 + EndpointManager 4 + SectionDelta 2 + 其它其余 是 ~20+ tests/-**dataplne 包下所有 cases**）；`common:compileJava` + `fabric:compileJava` GREEN under -Pmc_ver=1.20.1。
  - **Task 10: partial commit (docs done); TCP cleanup + cross-process smoke phase deferred to explicit Task 10b followup**
## 2026-07-27：统一端点模型计划

- 已完成 `docs/superpowers/plans/2026-07-27-unified-endpoint-model-and-nginx-smoke.md`：覆盖 TOML/ConfigSpec、immutable snapshot、append-only listener groups、Fabric/NeoForge 接线、客户端候选回退、恢复 marker 状态门、Nginx TCP 注入及九段验证。
- `udp-failover` 的 uf2 日志显示：真实 UDP client bind 成功；`FAILOVER_RECONNECT_OK epoch=0` 被初始握手错误触发；随后 client 在首批 `chunk_payload_s2c`/`aggregation` 后不再推进 smoke state。该挂起尚未定位为 dataplane 根因，不能以 smoke harness 结果宣称 failover 已通过。
