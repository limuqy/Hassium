...
public final class ForgeConfigBackend implements IConfigBackend {
    ...
    public ConfigValues load(ConfigScope scope) {
        ...
        values = loadScope(values, ConfigScope.CLIENT, clientValues);
        return loadScope(values, ConfigScope.SERVER, serverValues);
    }

    @Override
    public void save(ConfigScope scope, ConfigValues values) {
        writeScope(values, ConfigScope.CLIENT, clientValues);
        writeScope(values, ConfigScope.SERVER, serverValues);
        clientSpec.save();
        serverSpec.save();
    }
