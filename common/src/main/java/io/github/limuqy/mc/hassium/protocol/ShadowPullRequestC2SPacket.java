            ...
            for (int i = 0; i < entry.sectionHashes().size(); i++) {
                ...
                if (hash != 0L) {
                    int[] plane = entry.planes() != null && i < entry.planes().length
                            ? entry.planes()[i] : null;
                    for (int p = 0; p < io.github.limuqy.mc.hassium.protocol.sectiondelta.SectionPlaneSyndrome.PLANE_COUNT; p++) {
    ...
    public static ShadowPullRequestC2SPacket decode(FriendlyByteBuf buf) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < sectionCount; j++) {
                ...
                if (hash != 0L) {
                    int[] plane = new int[io.github.limuqy.mc.hassium.protocol.sectiondelta.SectionPlaneSyndrome.PLANE_COUNT];
