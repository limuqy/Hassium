...
import io.github.limuqy.mc.hassium.compression.ZstdRuntimeBridge.CompressDict;
import io.github.limuqy.mc.hassium.compression.ZstdRuntimeBridge.DecompressDict;
 ...
 * <p>
 * {@link CompressDict}（zstd-jni ZstdDictCompress）按 level 缓存，
 * {@link DecompressDict}（ZstdDictDecompress）单例缓存。
...
public class ZstdDictionaryCompressionCodec implements CompressionCodec {
    ...
    private final ConcurrentHashMap<Integer, CachedDictHandle<CompressDict>> compressCache = new ConcurrentHashMap<>();

    /** 解压字典句柄（单例缓存，volatile 保证可见性；字典内容变更时重建） */
    private volatile CachedDictHandle<DecompressDict> decompressHandle;
    ...
    public synchronized byte[] compress(byte[] input, CompressionOptions options) throws CompressionException {
        ...
        CachedDictHandle<CompressDict> cached = compressCache.get(level);
        CompressDict dict = (cached != null && cached.contentVersion == version) ? cached.handle : null;
        if (dict == null) {
            try {
                dict = ZstdRuntimeBridge.newCompressDict(dictionary, level);
            } catch (Exception e) {
            ...
            CachedDictHandle<CompressDict> old = compressCache.put(level, new CachedDictHandle<>(version, dict));
            if (old != null) {
                old.handle.close();
            }
        }
        ...
        try {
            return ZstdRuntimeBridge.compress(input, dict);
        } catch (Exception e) {
    ...
    public synchronized byte[] decompress(byte[] input, CompressionOptions options) throws CompressionException {
        ...
        // review-fix: T5-91 句柄创建失败包装 DictionaryLoadException（不逃逸裸 RuntimeException）
        long version = contentVersion(dictionaryId);
        CachedDictHandle<DecompressDict> cached = decompressHandle;
        DecompressDict dict = (cached != null && cached.contentVersion == version) ? cached.handle : null;
        if (dict == null) {
            try {
                dict = ZstdRuntimeBridge.newDecompressDict(dictionary);
            } catch (Exception e) {
                throw new CompressionException.DictionaryLoadException(dictionaryId,
            ...
            // review-fix: 覆盖旧句柄前先 close，防字典热替换后旧 native 句柄泄漏（缓存命中路径不 close，review §2.5）
            CachedDictHandle<DecompressDict> old = decompressHandle;
        ...
        try {
            ...
            if (contentSize > 0) {
                ...
                return ZstdRuntimeBridge.decompress(input, dict, (int) contentSize);
            }
            ...
            try (ZstdRuntimeBridge.DecompressCtx ctx = ZstdRuntimeBridge.newDecompressCtx()) {
                ...
                while (capacity <= MAX_DECOMPRESSED_SIZE) {
                    ...
                    } catch (ZstdRuntimeBridge.ZstdErrorException e) {
                        if (e.errorCode() != ZstdRuntimeBridge.errDstSizeTooSmall()) {
                            throw e;
                        }
