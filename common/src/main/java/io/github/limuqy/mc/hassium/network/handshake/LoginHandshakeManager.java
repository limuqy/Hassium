 ...
 * 登录期能力握手——服务端协商核心。
 * <p>
 * 双载体（1.20.1 login query / 1.20.2+ 配置阶段 payload）收敛到统一登记：
 * {@link #markNegotiated} 按玩家 UUID 写入 {@link PlayerCompressionTracker}，
 * ServerPlayer 物化时消费。协商位 = 服务端声明位 × 客户端声明位按位与。
 * <p>
 * 仅 1.20.1 login query 需要 per-connection 状态（应答与 query 的配对）；
 * 1.20.2+ 配置阶段 payload 直带 UUID（{@code getOwner()}，认证已完成）。
 */
