...
A: 不会立即掉线。L1 迁移引擎按生效静默超时（默认 `master.migrationSilentTimeoutMs`=`10000`；显式改 `migrationFaultTimeoutMs` 时可回退）判定后无感迁移：磁盘缓存、保存队列全保留，新会话直接续上，不弹「连接丢失」；主控无法恢复时才真正断连。可用 `/hassium migrate` 演练。UDP 数据面开启且健康时迁移更平滑。
