...
| 分类 | 能力 | 说明 |
| --- | --- | --- |
| **存储优化** | 高效存储 | 世界区块用更高压缩率落盘，显著减小存档体积；仍兼容原版 Region（`.mca`）布局 |
| **网络优化** | 网络压缩 | 区块与数据包用更高效压缩传输，降低带宽占用与下载等待 |
| | 平滑推送 | 服务端恒定速率限速（150 块/s 令牌桶）+ 主线程序列化上限与后台化；进服/扩展视野不卡主线程 |
...
| **超视渲染** | 超视渲染 | 多人服客户端 RD 大于服务端视距时，用本地缓存回填视距外地形（仅渲染、不向服索要视距外区块） |
