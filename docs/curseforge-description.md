...
| Export visited terrain | Client: `/hassiumc export [serverIp] [seed]` → `saves/` |
...
| `/hassiumc export [<serverIp>] [seed]` | Client | Export local cache as vanilla Anvil under `saves/` |
...
| 导出去过的地形 | 客户端 `/hassiumc export [服务器IP] [seed]` → `saves/` |
