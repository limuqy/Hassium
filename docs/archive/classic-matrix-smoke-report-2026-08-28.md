...
## 2026-08-29 NeoForge 全版本复跑明细

会话：`Phase I`，R1 `VD20`、R2 `VD10`，客户端 `renderDistance=32`，均为两轮完整结束且分析器 `failures=[]`。数据来自各会话的 `result_<SessionId>.json` 中 `Probe.Round1/Round2`，不是日志文本估算。

| 版本 | R1 landed | R2 landed | R2 整柱命中 | R2 delta | R2 整柱请求 | R2 OVD（loaded/miss） |
|---|---:|---:|---:|---:|---:|---:|
| 1.21.1 | 1529 | 453 | 443 | 10 | 0 | 632/0 |
| 1.21.2 | 1529 | 453 | 410 | 43 | 0 | 632/0 |
| 1.21.3 | 1529 | 453 | 425 | 28 | 0 | 632/0 |
| 1.21.4 | 1529 | 453 | 447 | 6 | 0 | 632/0 |
| 1.21.5 | 1529 | 453 | 442 | 11 | 0 | 632/0 |
| 1.21.6 | 1529 | 453 | 467 | 21 | 0 | 632/0 |
| 1.21.7 | 1529 | 453 | 426 | 27 | 0 | 632/0 |
| 1.21.8 | 1529 | 453 | 431 | 22 | 0 | 632/0 |
| 1.21.9 | 1529 | 453 | 424 | 29 | 0 | 632/0 |
| 1.21.10 | 1529 | 453 | 474 | 14 | 0 | 632/0 |
| 1.21.11 | 1529 | 453 | 432 | 21 | 0 | 632/0 |

字段口径：`landed` = `clientLandedChunkCount`；`整柱命中` = `cacheHitFullChunkCount`；`delta` = `sectionDeltaChunksReceived`；`整柱请求` = `fullChunkRequestCount`；`OVD` = `ovdLoaded/ovdPendingMiss`。R2 的 `landed=453` 包含整柱缓存命中与 delta 应用，因而与“区块加载：新增 0”不矛盾；新增整柱网络请求在 11 个版本均为 0。
