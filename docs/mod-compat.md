...
| 服务端进程内 Via 与 `master.globalPacketCompression` | 管线 ZSTD 仅对完成 Hassium 握手的玩家安装，未握手（含 Via 翻译目标）玩家仍走原版 Zlib，无帧假设冲突；若自测出现压缩帧干扰，关 `master.globalPacketCompression` 逃生口仍有效 |
...
| `master.globalPacketCompression` | 关全局 ZSTD |
| `master.enablePacketAggregation` | 关包聚合 |
| `master.compressionBlacklist` | 排除指定包 ID（第三方通道） |
| `dataplane.enabled` | 关 UDP/KCP 数据面（网关↔主控通道 bulk 载体；默认已关）；用于隔离 UDP 防火墙、NAT 或代理问题，控制/握手仍走网关帧连接 |
| `chunk.hassiumEngineEnabled` | 关 Hassium 引擎（服务端不剥光，光照随包自带；影子端相关功能关闭） |
| `chunk.sectionDeltaEnabled` | 关分段增量（过期改走全量） |
| `chunk.viewDistanceExtensionEnabled` | 关 超视渲染（恢复原版 RD 钳制） |
| `chunk.maxRenderDistance` | 超视渲染 / 有效 RD 上限（2–64） |
| `chunk.ovdUnloadDelaySecs` | 离开超视渲染环带后延迟卸载（秒） |
| `compat.requireClientMod` | 是否强制客户端装模组 |

UDP 数据面部署时，`udpListeners[*].bindHost` 只用于服务器本机监听，`reachableEndpoints` 才下发给客户端。公网部署必须填写客户端可达地址并放行 UDP 端口；禁止把 `0.0.0.0`、`::` 或内网 bind 地址写为 reachable endpoint。网关监听地址使用独立的 `master.controlReachableEndpoints`（主控核心；`endpoints[0]` 即网关端口，兜底 25566），不与 UDP 列表混用。详见 [`architecture.md`](architecture.md)。
...
- [ ] **超视渲染**：多人服 `view-distance=8`、客户端 RD=16，曾走过的地形环带可见；F3 无大量视距外 `ChunkDataRequestC2S` / `BlockEntityRequestC2S`；关闭 `chunk.viewDistanceExtensionEnabled` 后恢复原版钳制；断连重连无残留 renderOnly 标记
