...
| 目标 | 策略 |
|------|------|
| Bobby / 同类客户端视距外缓存 | **不兼容**；后续自研视距外显示 |
...
| ViaVersion | **有条件**（见 §5） |
| Sodium / Lithium / ModernFix / FerriteCore 等热门优化 | **希望兼容**（见 §6） |
| C2ME | **Soft Compatible**（见 §7） |
...
Sodium / Iris / Lithium / FerriteCore / ModernFix / EntityCulling 等通常不抢 Region 压缩类型与区块广播接管。
...
| **Configured** | 软兼容：Forge/NeoForge 可选（已自带 Cloth 配置屏）；Fabric 不依赖 |
| **Forge Config API Port** | Fabric **不使用**；仅 Forge **1.20.6** jiJ（ModConfigSpec 桥接）；NeoForge 原生 Spec |

配置文件：物理客户端为 `hassium-client.toml` + `hassium-common.toml`；专用服仅 `hassium-common.toml`。
...
- C2ME 默认模块 vs chunkio rewrite；`storage` 开/关对照  
- Sodium + `lightStrip` 开/关  
