...
| 目标 | 策略 |
|------|------|
| Bobby / 同类客户端视距外缓存 | **不兼容**；Hassium 自研 OVD（视距外显示），勿与 Bobby 同装 |
...
| `clientCache.viewDistanceExtensionEnabled` | 关 OVD 视距外显示（恢复原版 RD 钳制） |
| `clientCache.maxRenderDistance` | 渲染距离上限（Fog/内存约束） |
...
- [ ] **OVD 视距外显示**：多人服 `view-distance=8`、客户端 RD=16，曾走过的地形环带可见；F3 无大量视距外 `ChunkDataRequestC2S` / `BlockEntityRequestC2S`；关闭 `clientCache.viewDistanceExtensionEnabled` 后恢复原版钳制；断连重连无残留 renderOnly 标记
- [ ] **OVD + Sodium**：ViewArea 扩大后 OVD 区块正常 mesh（多数 Sodium 版本可跟，必要时补条件 Mixin）
- [ ] **OVD 边界替换**：真实区块到达 renderOnly pos 后无闪烁、无重复 enqueue  
