...
| B4 | 迁移命令注册 | ✅ 已完成 | `/hassium migrate`（list / migrate <host:port> / status）三端注册（fabric + neoforge 客户端命令；服务端执行提示由客户端上下文检查兜底）；T10 演练真实命令路径触发 | `command/HassiumCommandHandler.java`（L362-378 用法/实现）、`fabric/.../FabricHassiumCommand.java`（L81-98）、`neoforge/.../NeoForgeHassiumClientCommand.java`（L62-69）；证据：`work/T10-MigrationDrill-TASK.md`（会话 A/B + 1.20.1/1.21.11） |
