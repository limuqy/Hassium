...
使用 ZSTD 替代原版 Zlib，降低存档体积与区块传输带宽。支持 Fabric / Forge / NeoForge，覆盖 Minecraft 1.20.1–1.21.11。
...
| 能力 | 说明 |
| --- | --- |
| **Region 兼容存储** | 保留 `.mca` 外层，payload type `126` + ZSTD（含字典） |
| **网络压缩** | 自定义 `hassium:*` 通道；可选全局包压缩替换原版 Zlib |
| **客户端区块缓存** | 卸载写入本地 Region；服务端推送 `chunkHash`，命中则跳过全量下载 |
| **进服限流** | 主线程序列化 + 后台压缩；客户端主线程时间预算 / JoinBoost |
| **Bloom / sectionHashes** | 减少无效 IO；section 哈希组合为 chunkHash |
| **流量监控** | `/hassium stats`（服务端）、`/hassiumc stats`（客户端） |

当前版本：`1.0.0-beta`。API 与配置可能变更。
